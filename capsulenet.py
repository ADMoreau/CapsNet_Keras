    ...
    def generate(self):
        while True:
            if self.current_idx == 0:
                ...
                start = int(end)
                end = int(self.batch_size) + int(end)
            #Reformat #
            ...
            try:
                x_out = np.array(self.x[start: end], dtype = "float32")
            except:
                print(start, end)
                start = end
                end = end + self.batch_size
                x_out = np.array(self.x[start:end], dtype = "float32")
            x_out = x_out.reshape(-1, 250, 62, 1)
            temp_y = np.array(self.y[start:end], dtype = "float32").transpose()
            # convert all of temp_y into a one hot representation
            y_out = to_categorical(temp_y, num_classes = 10)
            self.current_idx += 1
            yield ([x_out, y_out], [y_out, x_out])
...
def CapsNet(input_shape, n_class, routings):
    ...
    # Layer 1: Just a conventional Conv2D layer, Austin: changed activation='relu' to 'elu'
    ...
    # Shared Decoder model in training and prediction, changed relu to elu
...
if __name__ == "__main__":
    ...
    model, eval_model, manipulate_model = CapsNet(input_shape=[250,62,1],
                                                  n_class=10,
